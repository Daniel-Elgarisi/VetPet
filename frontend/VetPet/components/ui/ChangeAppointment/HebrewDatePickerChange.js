...
const HebrewDatePickerChange = ({ onDateChange, dateForChange }) => {
  const [currentMonth, setCurrentMonth] = useState(() => moment().format('YYYY-MM-DD'));
  const [markedDates, setMarkedDates] = useState({});
  const [selectedDate, setSelectedDate] = useState('');

  useEffect(() => {
    const initialDate = moment(dateForChange, 'DD-MM-YYYY').format('YYYY-MM-DD');
...
export default HebrewDatePickerChange;
