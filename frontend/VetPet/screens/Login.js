...
import CustomLoginButton from "../components/ui/Login/CustomLoginButton";
...
const LoginScreen = () => {
  ...
  const [timeLeft, setTimeLeft] = useState(10); // Assume meant as seconds for simplicity
  ...
  const handleSubmitId = () => {
    performPostRequest("/auth/verify-id", { identity_number: id }, (response) => {
      setIsCodeSent(true);
      setTimeLeft(20);
  ...
  const handleVerifyCode = async () => {
    ...
    performPostRequest("/auth/verify-otp", { identity_number: id, code }, async (response) => {
      const ownerId = response.data.ownerId;
      try {
        await AsyncStorage.setItem('ownerId', ownerId.toString());
      } catch (error) {
        console.log('Failed to save ownerId:', error);
      }
